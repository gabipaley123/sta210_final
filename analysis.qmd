...
```{r echo = FALSE, warning = FALSE}
fcf_model_addtl_factors <- lm(filtered_cleaned_fcf$NEXT_10DAY ~ fcf_growth + filtered_cleaned_fcf$current_ratio_change,
                              data = filtered_cleaned_fcf)
tidy(fcf_model_addtl_factors)
```
##  Industry Interaction Term Model
```{r echo = FALSE, warning = FALSE}
fcf_model_ind_interaction_term <- lm(filtered_cleaned_fcf$NEXT_10DAY ~ Industry*fcf_growth, data = filtered_cleaned_fcf)
tidy(fcf_model_ind_interaction_term)
```

## Model Adjusting for Earnings Quality

```{r echo = FALSE, warning = FALSE}
fcf_model_earnings_qual <- lm(filtered_cleaned_fcf$NEXT_10DAY ~ fcf_growth + earnings_quality, data = filtered_cleaned_fcf)
tidy(fcf_model_earnings_qual)
```
```
